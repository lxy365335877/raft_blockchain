...
from fl.models.Fed import FedAvg
...
def train(uuid, epochs, start_time):
    ...
    if str(uuid) in args.poisoning_attackers:
        logger.debug(
            "Detected id in attackers' list: {}, manipulate local gradients!".format(
...
def train_count(epochs, uuid, start_time, train_time, w_compressed):
    ...
    lock.acquire()
    g_train_local_fl.models.append(w_decompressed)
    lock.release()
    if train_count_num == args.num_users:
        logger.debug(
            "Gathered enough train_ready, aggregate global model and send the download link."
        )
        # reset counts
        lock.acquire()
        train_count_num = 0
        ...
        logger.debug(
            "As a committee leader, calculate new global model hash: "
            + global_model_hash
        )
...
def round_finish(uuid, epochs):
    global global_model_hash
    logger.debug(
        "Received latest global model for user: %s, epoch: %s." % (uuid, epochs)
    )
    ...
    def main_handler():
        ...
        # For POST
            ...
            if message == "prepare":
                ...
                threading.Thread(
                    target=round_finish, args=(data.get("uuid"), data.get("epochs"))
                ).start()
    ...
    def trigger_handler():
        ...
        if request.method == "POST":
            ...
            if message == "train_ready":
                ...
                threading.Thread(
                    target=next_round_count, args=(data.get("epochs"),)
                ).start()
