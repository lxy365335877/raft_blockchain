...
from fl.models.Update import DatasetSplit


def test_img_new(net_g, ldr_test, device="cpu"):
    net_g.eval()
    # testing
    test_loss = 0
    correct = 0
...
def test_img(net_g, dataset_test, args, test_indices):
    ...
    data_loader = DataLoader(
        DatasetSplit(dataset_test, test_indices), batch_size=args.bs
    )
...
def test_lstm(net_g, dataset_test, args, test_indices):
    data_loader = DataLoader(
        DatasetSplit(dataset_test, test_indices),
        batch_size=40,
        shuffle=True,
        drop_last=True,
    )
