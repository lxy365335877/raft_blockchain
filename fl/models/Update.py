...
def train_new(net, ldr_train, local_ep, device, lr, local_bs):
    net.train()
...
def train_cnn_mlp(net, dataset, idxs, local_ep, device, lr, local_bs):
    ...
    ldr_train = DataLoader(
        DatasetSplit(dataset, idxs), batch_size=local_bs, shuffle=True
    )
    loss_func = nn.CrossEntropyLoss()
...
def train_lstm(net, dataset, idxs, local_ep, device):
    ...
    ldr_train = DataLoader(
        DatasetSplit(dataset, idxs), batch_size=40, shuffle=True, drop_last=True
    )
    ...
    for epoch in range(local_ep):
        ...
        for batch_idx, (data, labels) in enumerate(ldr_train):
            ...
            loss_train.backward()
            optimizer.step()
        avg_losses_epoch_train = sum(losses_epoch_train) / float(
            len(losses_epoch_train)
        )
...
def local_update(net, dataset, idxs, args):
    if args.model == "lstm":
        ...
        return train_cnn_mlp(
            net, dataset, idxs, args.local_ep, args.device, args.lr, args.local_bs
        )
