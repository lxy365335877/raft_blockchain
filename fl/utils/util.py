...
from fl.datasets.LOOP import LOOPDataset
from fl.datasets.REALWORLD import REALWORLDDataset
from fl.datasets.UCI import UCIDataset
from fl.models.Nets import CNNCifar, CNNMnist, CNNFashion, UCI_CNN, MLP, LSTM
from fl.models.test import test_img_total, test_lstm
from fl.utils.sampling import iid_onepass, noniid_onepass
...
def dataset_loader(dataset_name, dataset_train_size, isIID, num_users):
    dataset_test_size = int(
        dataset_train_size * 0.25
    )  # the dataset size ratio of the training to the test is 8:2
    logger.info(
        "Load dataset [%s] with training data size [%d], test data size [%d]"
    ...
    if dataset_name == "mnist":
        ...
        mnist_data_path = os.path.join(real_path, "/mnt/e/datasets/")
        dataset_train = datasets.MNIST(
            mnist_data_path, train=True, download=True, transform=trans_mnist
        )
        dataset_test = datasets.MNIST(
            mnist_data_path, train=False, download=True, transform=trans_mnist
        )
        if isIID:
            dict_users, test_users = iid_onepass(
                dataset_train,
                dataset_train_size,
                dataset_test,
                dataset_test_size,
            ...
            dict_users, test_users, skew_users = noniid_onepass(
                dataset_train,
                dataset_train_size,
                dataset_test,
                dataset_test_size,
        ...
        dataset_train = datasets.FashionMNIST(
            mnist_data_path, train=True, download=True, transform=trans_fashion
        )
        dataset_test = datasets.FashionMNIST(
            mnist_data_path, train=False, download=True, transform=trans_fashion
        )
        if isIID:
            dict_users, test_users = iid_onepass(
                dataset_train,
        ...
        dataset_train = datasets.CIFAR10(
            cifar_data_path, train=True, download=True, transform=trans_cifar
        )
        dataset_test = datasets.CIFAR10(
            cifar_data_path, train=False, download=True, transform=trans_cifar
        )
        if isIID:
            dict_users, test_users = iid_onepass(
                dataset_train,
...
def model_loader(model_name, dataset_name, device, num_channels, num_classes, img_size):
    logger.info(
        "Load model [%s] for dataset [%s]. Train using device [%s]."
...
def env_from_sourcing(file_to_source_path, variable_name):
    source = 'source %s && export MYVAR=$(echo "${%s[@]}")' % (
        file_to_source_path,
        variable_name,
    )
    dump = "/usr/bin/python3 -c \"import os, json; print(os.getenv('MYVAR'))\""
    pipe = subprocess.Popen(
        ["/bin/bash", "-c", "%s && %s" % (source, dump)], stdout=subprocess.PIPE
    )
...
def http_client_post(url, body_data, accumulate_time=True):
    logger.debug(
        "[HTTP Start] [" + body_data["message"] + "] Start http client post to: " + url
    )
...
def __compress_data(data):
    encoded = json.dumps(
        data, sort_keys=True, indent=4, ensure_ascii=False, cls=NumpyEncoder
    ).encode("utf8")
...
def generate_md5_hash(model_weights):
    ...
    data_md5 = hashlib.md5(
        json.dumps(np_model_weights, sort_keys=True, cls=NumpyEncoder).encode("utf-8")
    ).hexdigest()
    return data_md5
...
def shutdown_count(uuid, from_ip, fed_listen_port, num_users):
    ...
    if shutdown_count_num == num_users:
        ...
        for uuid in ipMap.keys():
            client_url = (
                "http://" + ipMap[uuid] + ":" + str(fed_listen_port) + "/trigger"
            )
