...
def train(user_id, epochs, w_glob_local, w_locals, w_locals_per, hyperpara, start_time):
    ...
    if epochs is None:
        ...
        logger.debug(
            "Downloaded initial global model hash: "
            + utils.util.generate_md5_hash(w_glob)
        )
    ...
    for iter in reversed(range(epochs)):
        logger.info(
            "Epoch [" + str(iter + 1) + "] train for user [" + str(user_id) + "]"
        )
        train_start_time = time.time()
        # compute v_bar
        for j in w_glob.keys():
            w_locals_per[j] = (
                hyperpara * w_locals[j] + (1 - hyperpara) * w_glob_local[j]
            )
    ...
    def trigger_handler():
        ...
        if request.method == "POST":
            ...
            if message == "fetch_user_id":
                ...
                threading.Thread(
                    target=average_local_w,
                    args=(
                        data.get("user_id"),
                        data.get("epochs"),
                        data.get("from_ip"),
                        data.get("w_glob_local"),
                        data.get("w_locals"),
                ...
                threading.Thread(
                    target=utils.util.shutdown_count,
                    args=(
                        data.get("uuid"),
                        data.get("from_ip"),
                        fed_listen_port,
...
def main():
    ...
    peer_address_var = utils.util.env_from_sourcing(
